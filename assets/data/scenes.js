...
export const SCENES = {
  ...
  /* 1. オフィス：机に座り、ノート PC を操作する男性
     ...
     既知の見え方の問題（2026-08-18 の全数検査で確認）：机の下端が接地 y と同じ 236 な
     ため、figure() の足の線（stroke-width 2.5 ＝接地yの上下に 1.25 ずつ広がる）が
     机の下辺からわずかにはみ出し、天板の下に黒い小片が見える。meeting-room（238）と
     kitchen（238）では下端を 1〜2 単位下げて解消済みなので、触る機会があれば揃えること。
     脚が隠れているため「座っている」の根拠は頭頂の高さ（接地 y−100）だけ。
     seated を問う設問はこの場面では作らないこと（musician・laboratory-seated を使う）。 */
  'office-desk': (() => {
  ...
  /* 2. 会議室：3 人が着席し、1 人がスクリーンの左端に手を触れて立っている
     （**2026-08-18 現在 vol6-l1.js No.4 が参照している。**「どの設問からも参照
     されない汎用場面／文言上の制約はない」と書かれていたが、それは Vol.6 作成前の
     記述で、いまは誤り。座標を動かすときは No.4 の
     「A man is touching the edge of a screen.」「テーブルの上には何も置かれて
     いない」「椅子を並べている人物が写っていない」と突き合わせること。
     No.4 は 2026-08-18 に**姿勢に依存しない 4 択へ作り替えた**ので、
     「3人は着席しており、立っているのは1人だけ」という旧版の根拠はもう使っていない）。

     **seated / standing を問う設問はこの場面では作らないこと**（office-desk と同じ制約。
     必要なら musician・laboratory-seated を使う）。2026-08-18、この注記が office-desk に
     しか無かったことが原因で、**同じ作りなのに meeting-room では seated を根拠にした
     誤答が書かれ、切れないまま残った**（vol6-l1.js No.4 の旧誤答
     'Three people are standing in a row.'）。理由は office-desk とまったく同じ:
     着席3体は共通の figure(sit) ポーズで、脚は nearFace の table の塗りに完全に飲まれる。
     残る根拠は頭頂 y（座 136 / 立 110）の差だけで、しかも 3 体は頭・首・胴（台形）しか
     見えないため、544px では「天板の上に台形が3つ並んでいる」とも読める。
     **絵に描かれた「座っている」の根拠が頭の高さ 1 つしかない場面では、姿勢を論点に
     しないこと。**接触・遮蔽・物の有無のように、幾何で一意に決まる事実を使う。
  ...
  /* 3. 工事現場：はしごの途中に立っている作業員
     figure(stand) の接地 y をはしご途中の段（rung y=148）に置くことで「昇っている途中」を表す。
     はしごは**壁に立てかけていない**（両側柱とも完全な垂直・右の建物 x280〜430 とは離れている）。
     drills/listening.js p1v-02 の誤答「A ladder is leaning against a wall.」を切る根拠。
     手の位置に注意：手は接地段の 2 段上（y=100）の桟を掴んでいるが、股関節（y=88）
     よりさらに 12 単位**下**なので、絵としては「腕を下ろして腰の高さの桟を握っている」
     ように見える。**「上の段に手を掛けて体を引き上げている」ようには描かれていない**
     ので、reaching up / pulling himself up 系の正解文は書かないこと。 */
  'construction': (() => {
  ...
  /* 4. カフェ：カウンターで注文する客と店員（部品版）
     ...
     現行 2 問は異なる幾何的根拠で成立している。次にこの場面で作問するときは
     どちらの根拠も再利用せず、3 つ目の根拠（例：棚のカップの有無、着席かどうか
     など）を選ぶこと。
     - drills/listening.js p1v-04（正解「An employee is reaching across a
       counter.」）の根拠は**店員の腕がカウンター前面を越えて伸びていること**
       （腕そのものの位置）。
     - vol6-l1.js No.6（id v6q6。2026-08-18、crosswalk から差し替え。正解
       「Two people are standing on opposite sides of a counter.」）の根拠は
       **2 人の前後関係**：店員は脚（y176〜236）がカウンター前面の塗り
       （x56–404 / y172–238）に完全に隠れて向こう側、客は足元の線 y=262 が
       カウンター下端 238 より 24 単位下に出て手前側（接地 y 差 262−236＝26）。
       差し替えの経緯・実測値は vol6-l1.js No.6 のコメントに詳しい。 */
  'cafe-counter': (() => {
  ...
  /* 6. 横断歩道：3 人が路上に横に並んで立っている（車・自転車は写っていない）
     **「渡っている」ではない。**3 体とも figure(…, 'stand') で、膝の曲げ 0・前方
     オフセット 0・両足とも同じ y=236 で接地・腕は 1 本も描いていない。歩行の
     手掛かりは絵の中に一つも無いので、正解文に crossing / walking は書けない
     （2026-08-18 まで本コメントが「渡っている」と書いており、それを信じて作られた
     vol3-l1.js No.4 が `Pedestrians are crossing the road.` を正解にしていた）。
     読み取れるのは「同じ高さの地面に 3 人が等間隔で並んで立っている」ことだけ。
     vol3-l1.js No.4 はその後「Three people are standing side by side.」
     （id v3q4r）に差し替え済みで、いまはこの記述と一致している。
     既知の見え方の問題：stripes は rect() ヘルパー（必ず stroke が付く）で描いて
     いるため、路面のペイントではなく「輪郭線付きの縦長の箱」に見える。
     **現在この場面を使うのは v3q4r の 1 問だけ。**vol6-l1.js No.6 は当初この
     場面を使う予定だったが、(1) v3q4r と同じ絵で「3人が並んで立っている」という
     同一の命題を2度正解にすることになる点、(2) 上記の stripes が縦長の箱に見える
     点、の2つを理由に cafe-counter へ差し替えられた（経緯は vol6-l1.js No.6 の
     コメントに詳しい）。次にこの場面で作問するときは「横に並んで立っている」以外の
     命題を立てられないか検討すること。 */
  'crosswalk': (() => {
  ...
  /* 7. 厨房：カウンター上のコンロにかけた鍋の取っ手に手をかけている調理人
     ...
     **既知の見え方の問題（2026-08-18）：鍋の取っ手 line(218,144 → 200,142) は
     544px で「取っ手」として読めない。**
     - この 18 単位の線は手の円（中心 200,142 / r=4.5）で終わっており、
       肩 →(186,148)→ 手 と続く nearAction の腕とほぼ一直線に並ぶ。画面では
       「腕がそのまま鍋まで伸びている」か「手が細い棒を握っている」ようにしか見えず、
       鍋の側面から生えた取っ手には見えない。
     - 鍋の胴（x219〜237）にも縁の楕円（cx228 rx11.5）にも、取っ手の付け根を示す線が無い。
     **したがって handle / lid など鍋の部位に依存する正解文は書かないこと**
     （laboratory の顕微鏡と同じ扱い）。vol6-l1.js No.1 は 2026-08-18 に、この理由で
     正解を 'A man is reaching for the handle of a pot.' から
     'A man is reaching for a pot on the stove.' へ直してある。
     **描き直すなら、取っ手は鍋の胴から一度上へ立ち上げてから水平に出す等、
     腕と別方向に振ること。**腕の延長線上に置く限り、線を太くしても長さを変えても
     「腕の続き」に見える。付け根に短い縦線を入れて胴との接続を明示するのも要る。 */
  'kitchen': (() => {
  ...
  /* 8. 実験室：作業台の奥に立つ研究者が顕微鏡の調整つまみに手を掛けている（部品版）
     ...
     既知の見え方の問題（2026-08-18 の全数検査で確認）：顕微鏡は輪郭線を入れたあとも
     「垂直の支柱＋斜めのアーム＋先端の小さな円＋台形の台座」という構成のままなので、
     544px の light / dark どちらでも**スタンドライトかマイクのブームに見える。**
     vol1-l1 No.6 は、この見え方の問題（顕微鏡と判別できないこと）を理由に
     2026-08-18 に正解を差し替えられた（旧 id v1q6 の正解「A researcher is
     adjusting a microscope.」→ 新 id v1q6r の正解「A researcher has one hand
     on a piece of equipment.」）。新しい正解は器具名を挙げず、伸ばした腕の先の
     手（276,149）が器具のつまみ（円 cx279 cy150 r6）に重なっている接触の事実
     だけで成立している。
     **microscope / eyepiece / stage など顕微鏡の部位に依存する文は書かないこと。**
     また垂らした腕（farShoulder→(222,164)→(214,190)）は胴の左側を斜めに横切るため、
     胴に線が 1 本入って見える。 */
  'laboratory': (() => {
  ...
  /* 8b. 実験室（着席版）：実験台の右端に着き、座っている研究者
     vol4-l1.js No.3「A researcher is seated at a workbench.」は vol1-l1.js
     No.6r（id v1q6r）「A researcher has one hand on a piece of equipment.」
     （立って器具に手を掛けている）と両立しないため、別 ID として分離した
     （第3段階の衝突解決）。vol1-l1 No.6 は 2026-08-18 に正解文を差し替えられて
     いるが、「立位」対「着席」という両立しない構図そのものは変わっていない。
  ...
  /* 9. 空港：立っている旅行者と、床に自立するスーツケース、着席した人がいるベンチ
     スーツケースは誰の手も触れていない。ベンチは「無人」ではなく人が座っている状態にする。
     **旅行者は「歩いている」のではない。**figure(…, 'stand') で膝の曲げ 0・前方
     オフセット 0・両足とも y=236 で接地・腕は 1 本も描いていない（crosswalk と同型）。
     walking / heading toward … のような歩行の描写は絵から読み取れないので正解文に
     使わないこと（2026-08-18 まで本コメントが「歩く旅行者」と書いていた）。
     奥の windowFrame は幅 400 の大きな枠なので、案内表示板と読まれうる。
     それを誤答に使うなら「更新している人が写っていない」側で切ること。 */
  'airport': (() => {
  ...
  /* 10. 図書館：4 段の棚のうち上から 3 段に物が並び、人物が棚に手を掛けている（棚は空にしない）
     既知の見え方の問題（2026-08-18 の全数検査で確認）：propBooks(x, y, …) の y は
     「下段ブロックの上端」なので、棚板 y の上に載せるには y−h1 を渡す必要がある
     （laboratory は propBooks(376,172,…) で天面 182 に正しく載せている）。ここは
     棚板と同じ y（94/138/182）を渡しているため、背表紙のある下段ブロックが棚板の
     10 単位**下**へはみ出し、棚板がブロックの中を横切る。544px では「棚の下に
     引き出し／トレーが差してある」ように見え、book と読み取るのは難しい。
     現行 2 問（vol6-l1 No.2 / drills p1v-06）は「棚に何かが載っている＝空ではない」
     ことだけで成立しているので影響はないが、**本の見た目（背表紙・書名・冊数）に
     依存する正解文は書かないこと。** */
  'library': (() => {
  ...
  /* 12. 川と橋：ボートが橋の近くに停泊している（人物なし）
     橋（deck を全幅に渡し、橋脚を水面下まで下ろす）で「橋が水面をまたいでいる」ことを、
     係留索（船から橋脚へ渡した細い線）で「ボートが停泊している」ことを、それぞれ絵の中に
     明示する（vol1-l1 No.5「Some vessels are tied up near a bridge.」／
     vol5-l1 No.1「A bridge extends across the water.」／
     drills p1v-07「Boats are docked along the shore.」の根拠）。

     2026-08-25 に是正（旧実装の欠陥は下記の申し送りに残す）。変更点:
     - **船体の台形が上下逆だった（底辺98が上辺72より広い）のを直した。**
       新しい船体は上辺98（デッキ）・下辺62（喫水線側）で上が広い。さらに左舷を
       ほぼ垂直（船尾）、右舷を大きく傾け（船首）にして、左右非対称の「舳先」を
       持たせた（対称の台形のままだと、向きを直しても「桶」に見える恐れがあったため）。
     - **橋を「天面線＋縦棒4本＋下面線」の格子（＝窓のある壁に見えていた構造）から、
       薄い橋桁（rect、高さ14）＋その上のガードレール支柱（等間隔11本、天面線なし）＋
       橋脚2本（水面 y=170 を跨いで y=205 まで下りる、脚の付け根に橋脚キャップ）に
       作り替えた。**橋桁の間（支柱の間）は塗りが無いので水面/空が透けて見え、
       「壁」ではなく「上に薄い板が渡してあり、脚だけが水面まで下りている」ことが
       読み取れる。
     - **各船から最寄りの橋脚へ細い係留索（stroke-width 1.5、構造線より明確に細い）を
       1本渡した。**「係留されている／停泊している」の根拠を、無人であることによる
       消去法だけでなく、絵の中の陽性の証拠（ロープでつながれている）でも持たせるため。
       係留索は `<line>` であり `<polyline>` ではないので、tools/scenecheck.mjs の
       動作腕（腕は`<polyline>` かつ stroke-width=3 のみを対象にする）には掛からない。
     - 帆・旗・キャビンは追加していない（帆は drills p1v-07 の誤答
       「Sails are being lowered.」を紛れさせる恐れがあるため見送り。マストは
       裸のまま＝「マストはあるが帆は張られていない」状態で残す）。
     - 人物は追加していない（vol1-l1 No.5・vol5-l1 No.1・drills p1v-07 の誤答は
       いずれも人の動作を述べており、無人であることで消去できる設計を崩さないため）。
     - `data-part="bridge"`（橋桁・支柱・橋脚一式）と `data-part="boat"`（船体・マスト・
       係留索、船ごとに1グループ）を付けた。tools/scenecheck.mjs の判定対象
       （box・figure-*・contact・cue の4種）のどれにも該当しないため新たに ERROR/WARN が
       増えることはないが、「この場面には data-part 属性が一切ない」という
       場面丸ごとの WARN（項目1/2/3/4/5/6/10/11/12 を判定できない旨の注記）は消える。

     申し送り（旧実装の記録・今後同じ間違いを繰り返さないため）：
     旧実装は台形 `M60,214 L74,196 L146,196 L158,214 Z`（底辺98 > 上辺72）で、
     実物の船は喫水側が狭いのに底が広く上下が逆だった。橋も y=118〜150 の帯を
     4本の縦線で仕切っただけの構造で、天面・底面の線と縦線が矩形の並びを作り、
     「大きな空箱／仕切り線の入った横帯」に見えていた（2026-08-24 のオブザーバー
     目視で確認）。**船・橋の「見た目」に依存する新しい正解文（帆・船首・アーチなど）は
     書かないこと。**現行 3 問はいずれも「人が 1 人も写っていない」ことで人物動作の
     誤答を消せる設計だが、正解文自体は船・橋を絵から積極的に特定できることを要求する
     ので、消去法だけに頼らないこと。 */
  'waterfront': frame(`
    ...
    <g data-part="bridge">
      ...
      ${[30, 70, 110, 150, 190, 230, 270, 310, 350, 390, 430]
        .map(x => line(x, 98, x, 84, SOFT, 1.5)).join('')}
      ${rect(198, 112, 24, 8)}${rect(202, 120, 16, 85)}
      ${rect(388, 112, 24, 8)}${rect(392, 120, 16, 85)}
    </g>
    <g data-part="boat">
      <path d="M66,214 L60,196 L158,196 L128,214 Z" fill="${FILL}" stroke="${INK}" stroke-width="2"/>
      ${line(100, 196, 100, 160, INK, 2)}
      ${line(150, 198, 202, 160, SOFT, 1.5)}
    </g>
    <g data-part="boat">
      <path d="M266,222 L260,204 L358,204 L328,222 Z" fill="${FILL}" stroke="${INK}" stroke-width="2"/>
      ${line(300, 204, 300, 168, INK, 2)}
      ${line(350, 206, 392, 168, SOFT, 1.5)}
    </g>
    ${line(0, 258, W, 258, AI, 1.5)}${line(0, 246, W, 246, AI, 1.5)}`, true),

  /* 13. 屋外市場：台に商品（カップ4個）が並べられ、脇に箱を提げた人物が立っている
     台の商品（state）と、箱を持った人物（action）は同一場面で両立する。
     **売り手（gx=130）は腕を 1 本も描いていない。**商品を並べる・量る・渡すといった
     売り手の動作を述べる文は絵から読み取れないので、誤答としてのみ使うこと。
     既知の見え方の問題（2026-08-18 の全数検査で確認）：日よけの左支柱 line(136,120→236)
     が売り手の頭・胴・右脚とほぼ完全に重なり、**支柱が 1 本も見えない**（日よけが
     右の 1 本だけで立っているように見える）。さらに売り手の頭（中心 130,119 / r=9）が
     日よけの下辺 y=120 に食い込んでおり、頭が日よけに貼り付いて見える（SPEC③違反）。 */
  'market-stall': (() => {
  ...
  /* 14. 講義：スクリーンの前で発表する人と、長椅子に着席した聴衆 3 人
     既知の見え方の問題（2026-08-18 の全数検査で確認）：発表者の接地 y=176（奥の床）と
     3 人目の聴衆 a3（gx=190・座位）の位置が近すぎ、**発表者の左脚（x≈202〜204,
     y116〜176）と足の横線（197〜211, y=176）が a3 の胴（y162〜200 の台形）を
     横切っている。**544px で「座っている人の胸に横棒が刺さっている」ように見える。
     人物どうしを重ねない（SPEC③）に反しているので、座標を触る機会があれば
     発表者を右へ逃がすこと。現行 2 問（vol5-l1 No.2 / drills p1v-08）の正誤には
     影響しないが、「聴衆が何人か」を数えさせる設問は作らないこと。 */
  'presentation': (() => {
  ...
  /* 15. ホテルのフロント：係員はカウンターの内側、箱を持った客は手前
     係員はカウンター前面（nearFace）より先（figure層）に描いて脚を隠す。
     客は frontFigure 層でカウンターより後に描き、全身を見せる。
     既知の見え方の問題（2026-08-18 の全数検査で確認）：
     - 客の頭（中心 340,119 / r=9）に、奥の棚の最下段の線 y=120（x292〜428）が
       重なっている（SPEC③違反）。
     - 箱の前面左端 x=272 はカウンターの右端 x=270 のわずか 2 単位右で、天面 172 は
       カウンター天面 176 より上。544px では「カウンターの右端に載った箱」にも読める。
       vol6-l1 No.3 の why(D)「カウンターの上には何も置かれていない」はこの点で
       きわどいので、カウンター上の有無を争点にする設問はこれ以上増やさないこと。 */
  'hotel-lobby': (() => {
  ...
  /* 17. 海辺：パラソル状の三角と、台形 2 脚（人はいない）
     drills/listening.js の p1v-03 は、この台形が「椅子」に見えないという見え方の
     問題を理由に 2026-08-18 に正解を差し替えられた（旧 id p1v-03 の正解「Chairs
     have been arranged near the water.」→ 新 id p1v-03r の正解「Some objects
     have been arranged side by side near the water.」）。台形は背もたれも脚も無い
     単なる四辺形で、544px の light / dark どちらで見ても「椅子」とは読み取れない
     （桶・箱に見える）。新しい正解は物の名前を出さず、「同じ形・同じ大きさの物が
     2つ、同じ底辺 y=214 の上に間隔を空けて水際に並べられている」という幾何的な
     事実だけで成立している。
     **台形が椅子だと読めることを前提にした新しい正解文は書かないこと。**
     パラソルも支柱が天蓋の頂点から出て天蓋を貫き、そのまま水面の帯（y150〜190）を
     縦断して y=214 まで伸びている＝「水の中に立っている傘」に見える。 */
  'beach': frame(`
  ...
  /* 21. スーパー：買い物客が通路でカートを押している
     **左右の棚（4 段ずつ）には商品を 1 つも置いていない。**「棚に商品が並んでいる／
     補充されている」型の正解文はこの絵からは書けない。逆に「棚が空である」を正解に
     すると、店内に商品が皆無という不自然な写真を肯定することになるので、
     vol2-l1 No.4 のように**棚には触れず、人物の動作だけで問う**こと。 */
  'supermarket': (() => {
  ...
  /* 22. 受付：診療所のカウンター。人物は3人——カウンターの内側に立つ受付係（x=150）、
     室内に立つ来院者（x=300）、右の長椅子に着席した人（x=400）。
     来院者はカウンター（x=60〜250）から 50 単位離れた場所に立っており、**カウンターに
     着いている・応対を受けているようには描かれていない。**「客がカウンターで手続きを
     している」型の正解文は書かないこと。
     この場面は 3 人とも腕を 1 本も描いていない（nearAction が無い）。したがって
     人物の動作を述べる文は正解にできず、位置関係（立っている／座っている／内側・手前）
     だけが根拠になる。 */
  'clinic-reception': (() => {
  ...
  /* 24. 工場：ベルトコンベヤの右端の脇に立つ作業員と、機械の下に積まれた箱の山
     箱の山には触れていない（手 312,185 と最寄りの箱の角 330,196 の距離は約 21 ＝
     SPEC②の非接触 14 以上を満たす）。
     **ただし「箱へ手を伸ばしている」ようには見えない。**腕は肩(298,142)→手(312,185)で
     水平から 72° の**ほぼ真下**を向いており、箱の方向（同 31°）とは 40° 以上ずれている。
     544px では「片腕を下ろして立っている作業員」に見える。vol3-l1 No.2 は、この
     裏づけの弱さを理由に 2026-08-18 に正解を差し替えられた（旧 id v3q2 の正解
     「A worker is reaching toward a stack of cartons.」→ 新 id v3q2r の正解
     「Cartons have been stacked on the floor.」）。新しい正解は reach に依存せず、
     床に積まれた箱の状態を完了受動態で述べる形にしている。
     **reach / hand / grasp を根拠にする新しい正解文はこの場面では作らないこと。**
     位置関係（vol4-l1 No.6「standing next to a conveyor belt」）と状態
     （vol5-l1 No.5「Boxes have been stacked near a machine.」・vol3-l1 No.2r
     「Cartons have been stacked on the floor.」）は絵から確実に読める。
     propMachine は床から浮いた位置（y88〜150）にあり、壁掛けの盤に見える点にも注意。 */
  'factory': (() => {
  ...
  /* 25. 塗装：壁の前に立ち、ローラーを持ち上げている作業員（右下に塗料の缶）
     壁は左半分（x30〜170）が --ai-wash で塗り済み、右半分が未塗装。**「塗っている」
     ことの主な根拠はこの塗り分けの境界線であって、道具の描写ではない。**
     ローラーは手（160,108）から**右上**の (176〜196, 82〜92) へ伸びており、
     ヘッドが手より作業員の体に近い＝柄の向きが実物と逆になっている。
     544px では「棒の先に小さな板が付いたもの」にしか見えないので、
     roller / brush など道具の種類を名指しする正解文・誤答文は書かないこと。
  ...
  /* 27. 荷積み：トラックの脇の地面に段ボール箱が積まれ、離れた場所の作業員が別の箱を持っている
     荷台の中ではなく地面に積む（荷台内に置くと窓のように見えるため）。
     作業員（gx=90）とトラック（x180〜450）は 90 単位離れており、**作業員はトラックに
     触れていないし、積み下ろしの動線上にもいない。**とはいえ「トラック＋積まれた箱＋
     箱を持った作業員」という組み合わせは unloading / loading と読まれやすいので、
     is being loaded / unloaded を誤答に置くときは「その作業をしている人物が写って
     いない」ではなく「作業員はトラックから離れており、荷台にも箱にも手を掛けていない」
     と、位置で切る根拠を書くこと。
     既知の見え方の問題（2026-08-18 の全数検査で確認）：抱えている箱（x76〜108,
     y194〜222）が作業員の両脚（x82〜98）を塗りで隠すため、脚が箱の上下で切れて見える。
     箱の下端 222 と接地 236 の 14 単位の隙間だけが「持ち上げている」根拠になっている。 */
  'loading-dock': (() => {
  ...
  /* 28. 銀行：カウンターを挟んで 2 人が同じ書類に手を掛けている（奥に窓）
     「向かい合っている」ことの根拠は figure() の dir ではない（dir は肩の 3 単位の
     ずれしか変えず 544px では見えない）。**両者の腕がカウンター中央の同じ書類へ
     内向きに伸びていること**が唯一の根拠。腕を消すと vol4-l1 No.2 の
     「Two people are facing each other across a counter.」は成立しなくなる。
     既知の見え方の問題（2026-08-18 の全数検査で確認）：客（gx=300）の頭（中心
     300,119 / r=9）が windowFrame(280,44,150,78) の下辺 y=122 に食い込み、
     窓枠の下辺が頭の左右で途切れて見える（SPEC③「奥の什器の輪郭を頭部に重ねない」違反）。
     なお中央の書類は --rule-soft の平行四辺形なので、trayやマット類にも読まれうる。
     「金を数える」型の誤答は、物ではなく**動作主／動作が写っていない**ことで切ること。 */
  'bank-teller': (() => {
