...
export const L1 = [
  ...
  p2(24, { t: ['p2ind'], lv: 5, sa: 'M-Cn', sb: 'W-Br',
    ...
    c: ['The original quotation for a smaller order went through last week.', 'They\'re waiting on a price from their fabricator.', 'Not by courier, I think, from the local printing office.'],
    a: 1,
    e: '時期を尋ねられ、「加工業者からの価格待ち」＝まだわからない理由を答える間接応答。',
    w: ['quotation の反復。先週通ったのは小口注文向けの元の見積もりで、これから届く修正版がいつになるかには触れていない。', '正解。', 'How への答え。配送手段を述べているだけで、修正版がいつ届くかには触れていない。'],
    ja: '修正した見積もりはいつごろいただけますか。→ (B) 加工業者からの価格待ちだそうです。' }),

  p2(25, { t: ['p2wh'], lv: 4, sa: 'W-Am', sb: 'M-Au',
    p: 'Isn\'t the archive open on Saturdays?',
    c: ['Only by appointment.', 'No, we haven\'t archived them.', 'It opens onto the courtyard.'],
    a: 0,
    e: '否定疑問に対し「予約制でのみ」と条件付きで肯定する応答。',
    w: ['正解。', 'archive の反復。資料の整理状況を述べており、開館日についての情報にならない。', 'open の別の意味を使った引っ掛け。'],
  ...
  p2(28, { t: ['p2ind'], lv: 5, sa: 'M-Br', sb: 'W-Am',
    ...
    c: ['Yes, we tested them.', 'I didn\'t pack the samples for the trade fair.', 'We only have twelve responses so far.'],
    a: 2,
    e: '「まだ 12 件しか回答がない」＝判断できる段階ではない、という間接応答。',
    w: ['test の反復。設問の test well with customers は「顧客の受けがよい」という自動詞の用法で、この応答の tested は他動詞。試験にかけたとだけ述べており、受けがよかったかどうかは何も言っていない。単数の the new packaging を them で受けている点も噛み合わない。', 'pack の反復。見本を詰めたのが自分かどうかという話で、顧客の評価がどうだったかには触れていない。', '正解。'],
  ...
  p2(29, { t: ['p2wh'], lv: 4, sa: 'W-Au', sb: 'M-Br',
    ...
    c: ['The induction starts on the fourth of September.', 'Half the intake has no reliable connection.', 'It was never held at the old regional training venue.'],
    a: 1,
    e: 'Why don\'t we ...? は提案。それに対する障害を挙げる応答が正解。',
    w: ['When への答え。開始日を述べているだけで、オンラインで実施するという提案の可否には触れていない。', '正解。', 'induction の照応。旧研修施設で開催されたことがあるかどうかという過去の話であり、今年オンラインで実施するという提案の可否には触れていない。'],
