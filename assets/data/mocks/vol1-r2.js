  ...
  sp({
    ...
    q: [
      ...
      { tag: '詳細', s: 'What is the problem with the original van?',
        c: ['Staff loaded it with the wrong pallets.', 'It cannot access the site.',
            'The depot needs it for another job.', 'Its lifting mechanism is not working.'],
        a: 3,
  ...
  sp({
    ...
    q: [
      { tag: '詳細', s: 'What is offered at no additional cost?',
        c: ['Use of a delivery van', 'Climate control offered in every storage unit', 'Access at any hour', 'A business account combined with the introductory rate'],
        a: 2,
        e: '「24 時間アクセスが追加料金なし。多くの事業者は夜間の入館を制限している」が根拠。',
        w: ['バンの貸し出しはない。', '空調ユニットは「利用可能」とあるだけで、全ユニット標準装備とは述べていない。', '正解。', '本文は法人料金と特典割引は併用できないと明記しており、両立するとは述べていない。'] },
      { tag: 'NOT', t: ['p7not'], s: 'What is NOT stated about the introductory offer?',
        c: ['It is limited to new business customers.', 'It is available to customers who complete their reservation online.',
            'It applies to units of a certain size or larger.', 'It covers the first four weeks at half the standard rate.'],
        a: 0,
  ...
  sp({
    ...
    q: [
      ...
      { tag: '詳細', s: 'What does Ms. Delacroix say about the four rolls?',
        c: ['They have not been set aside.', 'They are a different weight from the swatch.',
            'They arrived later than the others.', 'They came from the same dyeing run.'],
        a: 3,
        e: '「4 本は連番で、単一の染色ロットを示唆する」とある。ランダムな不具合ではない。',
        w: ['本文は I have set those four aside. と述べており、問題の 4 本はすでに取り分けてある。取り分けられていないとするこの記述は本文と正面から矛盾するので偽。', '重量ではなく色。', '納期の差には触れていない。', '正解。'] },
      { tag: '推測', t: ['p7inf'], s: 'What does Ms. Delacroix imply about contacting the dye house?',
        c: ['She has already sent them a complaint.', 'She wants to decide on a course of action first.',
            'She believes they will refuse to replace the rolls despite her call yesterday.', 'She thinks Rita should contact the dye house on her behalf.'],
        a: 1,
        e: '「まだ伝えていない。先にこちらの立場を決めたい。切り出せば向こうはどうしてほしいのか聞いてくるから」が根拠。',
        w: ['まだ伝えていないと明記。', '正解。', 'まだ染料工房に伝えていないと本人が明記しており、前日に電話した上での拒否予想という記述と矛盾する。', '「once I raise it（こちらが切り出せば）」と書いており、連絡は自分が行う前提である。リタに代わって連絡させるという記述はない。'] },
    ],
  }),
  ...
  sp({
    ...
    q: [
      { tag: '詳細', s: 'What is specified about the adapter?',
        c: ['It must support a particular connector.', "It comes from the client's own stock.",
            'The event needs two.', 'It is unavailable on the requested date.'],
        a: 0,
