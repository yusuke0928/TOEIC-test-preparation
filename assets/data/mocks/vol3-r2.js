    ...
    q: [
      ...
      { tag: '詳細', s: 'What is indicated about the Wednesday storytime sessions?',
        c: ['They will be cancelled if the weather is poor.', 'They will require advance registration through the online booking system.',
            'They will be shortened due to limited space.', 'They will be held indoors in the first-floor community room instead.'],
        a: 0,
        e: '屋外の中庭に移すが、雨天の場合は屋内に移さず中止すると明記されている。',
        w: ['正解。', 'おはなし会に事前登録が要るとは書かれていない。オンラインで受け付けているのは資料の取り置き（holds）であって、おはなし会の申し込みではない。', 'おはなし会は屋外の中庭に移るため、一階会議室の座席不足は関係がない。時間を短縮するという記載もない。', '一階の会議室に移るのは受付窓口と貸出業務であり、おはなし会は屋外の中庭に移る。'] },
    ],
  }),
    ...
    q: [
      { tag: '詳細', s: 'Why was the inspection date changed?',
        c: ['The inspector is unavailable on the originally planned date.', 'Staff requested a different date to suit their shift patterns.',
            'The warehouse will be closed for renovation on 14 March.', 'A required spare part was delayed by two weeks.'],
        a: 0,
      ...
      { tag: '詳細', s: 'What should a supervisor do if a forklift is still needed for loading at 1:00 p.m.?',
        c: ['Cancel that vehicle\'s inspection for the rest of the quarter.', 'Reschedule the entire inspection to another day.',
        ...
        w: ['検査を取りやめるのではなく、監督者が検査員と直接、短い例外対応を調整すると明記されている。', '全体の日程変更ではない。', '正解。', '恒久的な稼働停止の即時報告ではない。稼働停止は不合格時の話であり、使用中に時刻を過ぎた場合は例外対応の対象になる。'] },
    ],
  }),
    ...
    q: [
      { tag: '詳細', s: 'Why does the shop not offer an express option for leather rebinding?',
        c: ['The leather requires a multi-week conditioning process.', 'Customers rarely request faster service.',
            'Skilled staff are not always available.', 'Leather is difficult to obtain.'],
        a: 0,
        e: '革は約 3 週間かけて段階的に養生する必要があり、それを急ぐと 1 年以内の破損の主因になると説明されている。',
        w: ['正解。', '需要の話ではない。', '人員の話はない。', '入手性には触れていない。'] },
      { tag: '推測', t: ['p7inf'], s: 'What is suggested about cloth rebinding?',
        c: ['It is offered only to institutional customers, not to individuals visiting in person.', 'It costs more than leather rebinding for books of a similar size.',
        ...
        w: ['「対面でも郵送でも無料の状態診断」とあり、個人客も対象と読める文章。', '布（£58〜）は革（£96〜）より安価で、サイズにかかわらずむしろ安く済む。', '正解。', '布には革のような養生期間が不要と明記されている。段階的に養生してから裁断するのは革の工程であり、布はその工程を経ない。'] },
    ],
  }),
  ...
  sp({
    ...
    q: [
      ...
      { tag: '詳細', s: 'What was unusual about the tracking of the change?',
        c: ['It continued for three years rather than the more typical shorter period.', 'It measured costs rather than stoppages.',
            'It covered every line in the factory from the outset.', 'It was conducted by an outside hospital consultant.'],
        a: 0,
        e: '「この程度の小さな変更にしては珍しく」3 年間の追跡調査を行った点が特異だと述べている。',
        w: ['正解。', '測定対象は停止件数。', '当初は 1 ラインのみで開始。', '外部コンサルタントの話はない。'] },
  ...
  sp({
    ...
    q: [
      { tag: '詳細', s: 'What condition applies to 24-hour access?',
        c: ['It is limited to two named people per member account.', 'It requires annual confirmation from the safety officer.',
        ...
        w: ['人数の上限はパス発行枚数（最大4枚）の話であり、24 時間アクセス固有の条件ではない。', '書面で確認するのは工房会員であって安全担当者ではない。年次の再確認も定められていない。', '追加料金の記載はない。', '正解。'] },
  ...
  sp({
    ...
    q: [
      ...
      { tag: '推測', t: ['p7inf'], s: 'What does the final paragraph suggest?',
        c: ['Averages can conceal distinct groups within the data.', 'Customer surveys are more reliable than claim data.',
            'Warranty periods should be shortened.', 'Kitchen appliances differ from other product categories.'],
        a: 0,
        e: '「1 つの平均が、まったく異なる挙動をする 2 つの集団を静かに合わせてしまうことがある」という結びが根拠。',
        w: ['正解。', '調査手法の比較はしていない。', '保証期間の話は出ていない。', '「オーブンの話ではない」と明言している。'] },
    ],
  }),
];
