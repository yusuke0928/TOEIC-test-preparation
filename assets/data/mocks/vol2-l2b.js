    ...
    q: [
      ...
      { tag: '意図', t: ['p3int'], s: 'Why does the woman say, "I asked twice, in different words"?',
        c: ['To show the candidate misunderstood the question both times.', 'To indicate the response was not simply nervousness.',
            'To explain why the interview overran by nearly twenty minutes.', 'To suggest that the original question was not repeated.'],
        a: 1,
        e: '直前に「面接の緊張かもしれない」と言われたことへの応答。2 回試して同じだったので緊張では説明できない、という趣旨。',
        w: ['誤解の指摘ではない。表現を変えて 2 回尋ねて同じ答えが返ったことを、緊張では説明できない根拠として挙げている。', '正解。', '面接時間の超過には一切触れていない。この面接が予定を超過したという話ではなく、超過するとすれば今後設定される二次面談（各 30 分）の話である。', '女性は "I asked twice, in different words" と述べており、同じ問いを表現を変えて 2 回繰り返している。質問を繰り返していないとするこの記述は本文と正面から矛盾する。'] },
     ...
     のまま）。No.66 は変更していない。 */
  {
    ...
    questions: [
      ...
      { id: 'v2q66r', no: 66, tag: '意図', stem: 'What does the woman mean when she says, "Hold off for now"?',
        choices: ['She worries the added cost could change his decision.', 'She thinks they booked the wrong room initially.', 'She wants the venue to change the layout entirely.', 'She does not think the room meets the client\'s conditions.'],
        answer: 0,
        exp: '直後で「追加料金をまだ見せていないので、決定が変わるかもしれない」と理由を続けている。部屋の予約し直しやレイアウト変更、日程の話ではなく、費用の承認が先だという趣旨。',
        why: ['正解。', '男性はこれから「今日の午後に予約する」と言った段階で、予約はまだ行われていない。取り違えて予約し直すという話ではない。', '女性は「丸テーブルの部屋」という条件で部屋を絞っており、会場に配置を変えさせる話は出ていない。直後に続く理由も追加料金の提示であって、レイアウトではない。', '女性が挙げた 2 条件を受けて男性が "That only leaves one option, then." と結論しており、女性はそれを訂正せず予約の保留だけを求めている。条件に合う部屋がないとするこの記述は本文と正面から矛盾する。'],
        topics: ['p3int'] },
