  ...
  sp({
    ...
    q: [
      { tag: '詳細', s: 'Why will the sauna be closed?',
        c: ['For a change of ownership announced earlier in the year', 'For staff training required after new equipment was installed',
      ...
      { tag: '詳細', s: 'What will happen automatically for members with sauna-inclusive membership?',
        c: ['They will receive a partial refund.', 'They will receive a free guest pass.',
            'They will be upgraded to a higher membership tier.', 'They will have five days added to their membership.'],
        a: 3,
  ...
  sp({
    ...
    q: [
      ...
      { tag: '詳細', s: 'What is stated about the punch-card machines?',
        c: ['They will be kept indefinitely as a permanent backup system for emergencies.', 'They will be moved to a different entrance and kept in service there.',
        ...
        w: ['「1 月 27 日以降は完全に撤去する」とあり、恒久的に予備として残るわけではない。', '両方の入口の打刻機が端末に置き換わり、27 日以降は撤去される。別の入口に移して使い続けるという記述はない。', '正解。', '撤去はまだ先の話で、このメモの時点（1 月 14 日）では旧式機はまだ稼働している。'] },
    ],
  }),
  ...
  sp({
    ...
    q: [
      ...
      { tag: '推測', t: ['p7inf'], s: 'What is suggested about spot repairs?',
        c: ['They require the same four-day steaming of the fabric before any cutting.', 'They cost more than full reupholstery.',
        ...
        w: ['スポット修理は大きな生地パネルの張替えを伴わず 1 週間程度で終わるため、裁断前 4 日間の蒸し工程は不要と読める。', 'スポット修理（£45〜）は全面張替え（£340〜）より安価。', '常連客限定とは述べていない。', '正解。'] },
    ],
  }),
  ...
  sp({
    ...
    q: [
      { tag: '概要', s: 'Why is Ms. Oyelaran writing?',
        c: ['To decline a landscaping project due to a scheduling conflict this month', 'To confirm a completion date',
            'To request payment for the sketches and revised schedule now attached', 'To propose a change to an agreed planting scheme'],
        a: 3,
        e: '「南北とも同じ低い生垣」という当初の依頼に対し、2 通りの植栽に分ける提案をしている。',
        w: ['スケジュールの都合で辞退したのではなく、変更案を提案し続けている。', '完了日の確認ではない。', '添付の図面と改訂スケジュールは提案の裏付けで、末尾で求めているのは 9 パーセント増を認めるかどうかの判断。支払いを求める記述はどこにもない。', '正解。'] },
      ...
      { tag: '推測', t: ['p7inf'], s: 'What does Ms. Oyelaran imply about using birch?',
        c: ['It would exceed the planting budget on its own.', 'It is difficult to obtain locally because most nurseries do not stock it.',
            'It would fail to provide shade exactly when it is most needed.', 'It is unsuitable for a courtyard setting in general due to its size.'],
        a: 2,
        e: '「シラカバは 1 年の半分近く葉を落とすため、最も日陰が必要な夏に日陰を提供できない」という趣旨。',
        w: ['予算増の理由は高木種全般のコスト。', '入手の難しさには触れていない。シラカバを避ける理由として挙げられているのは落葉の時期だけである。', '正解。', '不向きとされているのは季節による落葉のタイミングであり、規模や中庭という設定自体ではない。'] },
    ],
  }),
  ...
  sp({
    ...
    q: [
      { tag: '詳細', s: 'What change did Brindlewood Insurance make in 2020?',
        c: ['It shortened the required opening script from five points to three.', 'It reduced the number of agents handling calls on each daily shift.',
            'It replaced a fixed script with required points in agents\' own words.', 'It began recording all customer calls for the first time using new software.'],
        a: 2,
        e: '固定の冒頭スクリプトをやめ、必須の 3 項目を担当者自身の言葉で伝える方式に変えたとある。',
        w: ['冒頭スクリプトを短くしたのではなく、読み上げの義務そのものを廃して、必須 3 項目を担当者自身の言葉で伝える形に置き換えたと本文は述べている。5 項目から 3 項目へ減らしたという記述もない。', '「人員・通話量・取り扱う商品に変更はない」と明記されており、人員削減はしていない。', '正解。', '録音自体は変更前から行われていたことが、変更のきっかけとなったレビューの前提になっている。新規導入ではない。'] },
      { tag: '詳細', s: 'What was unusual about the tracking of the change?',
        c: ['It continued for two years rather than the more typical shorter period.', 'It measured revenue rather than call length.',
            'It covered every department from the outset.', 'It was conducted by an outside consultancy.'],
        a: 0,
        e: '「この種の変更にしては珍しく」2 年間の追跡調査を行った点が特異だと述べている。',
        w: ['正解。', '測定対象は通話時間と再架電率。', '当初は 1 部門のみで開始。', '外部委託の話はない。'] },
    ...
    q: [
      { tag: '詳細', s: 'What did Fennimore Express discover about its missed deliveries?',
        c: ['They differed sharply between two types of address.', 'They came mainly from a small group of repeat addresses.',
            'They were higher than the industry average, matching the sixteen percent headline figure.', 'They were concentrated on a single weekday each month.'],
        a: 0,
        e: '戸建てと集合住宅とで、初回不在率が 9 パーセントと 31 パーセントに分かれた。',
        w: ['正解。', 'リピート住所の話ではない。', '挿入文で「その数字自体は 16 パーセントで平凡だった」と明言されており、業界平均より高いという記述とは矛盾する。', '曜日別の話は出ていない。'] },
      ...
      { tag: '推測', t: ['p7inf'], s: 'What does the final paragraph suggest?',
        c: ['Averages can conceal distinct groups within the data.', 'Customer surveys are more reliable than delivery data.',
            'Missed-delivery rates should not be published.', 'Delivery services differ fundamentally from other industries.'],
        a: 0,
        e: '「小包の話ではない。1 つの平均が、まったく異なる挙動をする 2 つの運用を静かに合わせてしまうことがある」という結びが根拠。',
        w: ['正解。', '調査手法の比較はしていない。', '公表の是非には触れていない。', '「小包の話ではない」と明言している。'] },
    ],
  }),
];
