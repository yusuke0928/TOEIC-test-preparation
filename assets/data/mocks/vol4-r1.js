...
export const R1 = [
  ...
  p5(103, { t: ['vform'], lv: 5,
    ...
    c: ['will be pushed', 'will push', 'are pushed', 'pushed'],
    a: 0,
    e: 'ファームウェアは「配信される」側なので受動態。once 節（現在形で未来を表す）に続く主節は未来形。',
    w: ['正解。', '能動態。', '受動態だが、主語 The updated firmware は不可算名詞で単数扱いなので、複数の主語に対応する are とは一致しない。', '過去形で時制が合わない。'],
  ...
  p5(111, { t: ['rel'], lv: 5,
    ...
    c: ['what', 'which', 'that', 'on which'],
    a: 3,
    e: 'rely on の on が前に出た形。空所の後ろが完全文なので前置詞＋関係代名詞。',
    w: ['先行詞がある場合は使えない。', '後ろが完全文なので不可。', '後ろが完全文なので不可。', '正解。'],
  ...
  p5(116, { t: ['confuse'], lv: 5,
    ...
    c: ['principal', 'principle', 'principally', 'principled'],
    a: 0,
    e: 'principal「主要な、主たる」（形容詞）。同音の名詞 principle「原則」との識別が要点。',
    w: ['正解。the principal reason「主たる理由」。',
  ...
  { id: 'v4-p6-131', part: 6, kind: 'doc', topics: ['connect', 'voice'], level: 5, docCount: 1,
    ...
    questions: [
      { id: 'v4q131', no: 131, stem: null, tag: '態・時制', topics: ['voice', 'ctense'],
        choices: ['must be disclosing', 'must be disclosed', 'was being disclosed', 'disclosing'],
        answer: 1,
        exp: 'disclose は目的語を必要とする他動詞だが、空所の後ろは to 以下の前置詞句だけで目的語が無い。目的語にあたる any potential conflict of interest が主語に立った受動態が入る。12 月 1 日からの規定なので義務を表す must を伴う。',
        why: ['能動の進行形。disclose は目的語となる名詞句を必要とするが、空所の後ろに目的語が無い。',
              '正解。must be disclosed。disclose の目的語にあたるものが主語に立った受動態で、義務を表す must に続く。',
              '受動態だが過去進行形。メールの日付は 11 月 4 日で、From 1 December はこれから始まる規定を指す。同じ文の before a client meeting takes place も現在形で、過去の出来事を述べる文ではない。',
  ...
  p6({
    ...
    q: [
      { tag: '態・時制', t: ['voice', 'ctense'],
        c: ['must be reporting', 'must be reported', 'have been reported', 'reporting'],
        a: 1,
      ...
      { tag: '態', t: ['voice'],
        c: ['must also be reporting', 'must also be reported', 'were reported', 'also reporting'],
        a: 1,
        e: 'report は目的語を必要とする他動詞だが、空所の後ろは to 以下の前置詞句だけで目的語が無い。目的語にあたる Any near miss ... が主語に立った受動態が入る。副詞 also は最初の助動詞 must の直後に置く。',
        w: ['能動の進行形。report は目的語となる名詞句を必要とするが、空所の後ろに目的語が無い。目的語を取らない report to + 人 は「〜の指揮下に入る、〜に出頭する」の意味で主語は人になる型なので、出来事である near miss はその主語に立てない。',
            '正解。must also be reported。report の目的語にあたるものが主語に立った受動態で、also は最初の助動詞 must の直後に置く。',
            '受動態だが複数の were。主語 Any near miss は単数なので数が合わない。過去形である点も、2 月 1 日から始まる規定を述べるメモの文脈と合わない（as well as が付加の意味を担うので、also が無いこと自体は誤りの理由にならない）。',
            '分詞。この節には定形動詞が他に無く、文の述語動詞になれない。'] },
      ],
  }),
  ...
  p6({
    ...
    q: [
      ...
      { tag: '態', t: ['voice'],
        c: ['will be stripped', 'will be stripping', 'has been stripped', 'stripping'],
        a: 0,
        e: 'strip A of B「A から B を剥奪する」は of の前に目的語を置く他動詞の型だが、空所の後ろは of the privilege だけで目的語が無い。目的語にあたる Staff who ... が主語に立った受動態が入る。4 月 1 日から適用される措置なので未来形。',
        w: ['正解。will be stripped。strip の目的語にあたるものが主語に立った受動態で、これから適用される措置なので未来形。',
            '能動の未来進行形。strip は of the privilege の前に目的語となる名詞句を必要とするが、空所の後ろに目的語が無い。',
            '受動態だが単数の has。関係節が Staff who share their ... と複数で受けているとおり主語 Staff は複数扱いで、数が合わない。現在完了である点も、4 月 1 日から適用される措置を述べる文脈と合わない。',
            '分詞。この節には定形動詞が他に無く、文の述語動詞になれない。'] },
      ],
  }),
];
