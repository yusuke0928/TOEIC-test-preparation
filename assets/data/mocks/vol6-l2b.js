  ...
  set({
    ...
    q: [
      ...
      { tag: '次の行動', s: 'What will the speakers most likely do this afternoon?',
        c: ['Use Bays One and Two for outbound loading.', 'Wait until Bay Three is repaired before resuming work and log it.', 'Move all inbound trucks into Bay Three once it reopens tomorrow morning.', 'Fit a replacement motor using a spare from the van and test it.'],
        a: 0,
        e: '「午前は搬入、午後は搬出に切り替える」という提案に同意しており、午後はバイ1・2を搬出に使う。',
        w: ['正解。', '修理を待たず代替策で進める方針になっており、記録を取るという話も出てこない。', 'バイ3はロープを張って誰も触れないようにすると述べられている。また、稼働できるのは早くとも明日の朝であり、今日の午後の話ではない。', '交換部品は車載在庫に無く翌日配送だと述べられており、当日試すという話も出てこない。'] },
    ],
  }),
  ...
  set({
    ...
    q: [
      { tag: '概要', s: 'What is the purpose of the woman\'s call?',
        c: ['To ask about delivery timing.', 'To request a refund for a cancelled booking.', 'To report that a delivered item is incomplete.', 'To report that an item arrived damaged.'],
        a: 2,
      ...
      { tag: '次の行動', s: 'What does the woman say she will do?',
        c: ['Postpone the doors opening until six-thirty in case of delay.', 'Call another supplier for a spare part.', 'Move the backdrop to a different room.', 'Station someone at the loading entrance.'],
        a: 3,
        e: '「配送のドライバーが迷わないよう、荷受け口で誰かに見ていてもらう」と述べている。',
        w: ['開場は6時のままで、女性は「それまでに届くか」と尋ねているだけである。遅延に備えて開場を遅らせるという案は誰からも出ていない。', '別の業者へ発注する話は出ていない。', '男性が勧めているのはフレームを寝かせておくことで、別の部屋へ移す話は出ていない。', '正解。'] },
    ],
  }),
  ...
  set({
    ...
    q: [
      ...
      { tag: '詳細', t: ['p3detail'], s: 'What will happen if the floor resurfacing runs over schedule?',
        c: ['The entire move-in process will be postponed until the next available day.', 'An additional fee will be charged to the exhibitor.', 'The dock being resurfaced will stay closed for the rest of the week.', 'The truck will be redirected to a different dock that morning.'],
        a: 3,
        e: '「予定を超えた場合は、その朝だけDock Twoに回す」と説明している。',
        w: ['翌日への延期については会話のどこにも出てこない。超過した場合の対応は「その朝だけ別の搬入口に回す」であり、搬入全体を翌日に延期するとは述べていない。', '追加料金の話は出ていない。', '超過した場合の対応は「その朝だけ Dock Two に回す」であり、補修中の搬入口が週内ずっと閉鎖されるとは述べていない。', '正解。'] },
    ],
  }),
];
