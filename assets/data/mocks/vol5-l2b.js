  ...
  set({
    ...
    q: [
      { tag: '詳細', s: 'Why are the banners delayed?',
        c: ['Nobody placed the order in time.', 'A print shop closed unexpectedly.',
    ...
    questions: [
      ...
      { id: 'v5q61', no: 61, tag: '詳細', stem: 'What must happen before the employee can use Zone A?',
        choices: ['A doctor\'s note must be submitted in person.', 'Facilities must approve the request.',
            'He must pay an additional deposit.', 'Six months must pass first.'],
        answer: 1,
        exp: '「施設管理の承認が必要」と述べられている。',
        why: ['担当者が挙げた条件は施設管理の承認だけで、診断書の提出は会話のどこにも出てこない。', '正解。', '追加保証金の話はない。', '条件として挙がったのは施設管理の承認（"that has to be approved by facilities"）だけで、申請は "I\'ll submit the request today"、男性も "Let\'s do that for now" と今から使う前提で頼んでいる。"six weeks" は待つ期間ではなく、Zone A を使える期間の上限（"it\'s usually for a maximum of six weeks"）である。'],
        topics: ['p3detail'] },
    ],
  },
