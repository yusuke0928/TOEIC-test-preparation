    ...
    q: [
      ...
      { tag: '詳細', s: 'What does Mr. Wysocki ask Ms. Achebe to do?',
        c: ['Ask the caterer to switch to the cold menu.', 'Find a replacement caterer for the entire event.',
            'Delay the start of seating until the caterer arrives.', 'Tell the caterer not to load the van yet.'],
        a: 0,
        e: '「今すぐ電話して、積み込む前に冷製メニューに切り替えるよう伝えて」と依頼している。',
        w: ['正解。', '代替業者の手配は依頼していない。', '開始時刻の変更は述べていない。', '本文の依頼は "call them now and tell them to switch before they load the van" であり、依頼されたのはメニューの切り替えであって、積み込みの中止ではない。'] },
    ],
  }),
    ...
    q: [
      { tag: '概要', s: 'Why is Ms. Rasmussen writing?',
        c: ['To request payment for a site visit completed last month', 'To confirm a delivery date for the ornamental trees',
            'To propose a change to an agreed planting plan', 'To confirm she has not withdrawn from the project'],
        a: 2,
        e: '「単一樹種」という当初の依頼に対し、2 樹種に分ける提案をしている。',
        w: ['費用請求ではない。現地は見ているが、その費用も実施時期も本文に出てこない。', '納期の確認ではない。樹木の納品日への言及自体が本文にない。', '正解。', '本文で「I have attached sketches of both options and a revised planting schedule」と述べており、計画に関わり続けていることがうかがえる。辞退していないという内容自体は本文と一致するが、目的として問われているのは提案の変更内容であり、辞退の有無ではない。'] },
      ...
      { tag: '推測', t: ['p7inf'], s: 'What does Ms. Rasmussen imply about matching the reception planting exactly?',
        c: ['It is not permitted under the terms of the original design brief.', 'It would not keep the cost within the original budget.',
        ...
        w: ['禁止規定の話は出てこない。彼女は「厳密に合わせると北側の花壇の可能性を無駄にする」と、可否ではなく得失として述べている。', '本文は "The two-species approach adds around ten percent to the planting cost, since it requires two separate supplier orders rather than one bulk order." と述べており、単一種でそろえるほう（＝厳密に合わせる案）が費用は低い。厳密に合わせると予算内に収まらないとするこの記述は本文と正面から矛盾する。', '正解。', '入手性には触れていない。'] },
    ],
  }),
  ...
  sp({
    ...
    q: [
      ...
      { tag: '詳細', s: 'What was unusual about the tracking of the change?',
        c: ['It was conducted by an outside retail consultant.', 'It did not cover the first six months.',
        ...
        w: ['外部コンサルタントの話はない。', '第3段落は The first six months showed a predictable drop in evening revenue of around eight percent と、最初の 6 か月ぶんの数字を示している。最初の 6 か月が対象外だったとするこの記述は本文と正面から矛盾するので偽。', '2 号店は昨年開業した別の話。', '正解。'] },
