  ...
  sp({
    ...
    q: [
      { tag: '詳細', s: 'Why are cars 3 and 4 being taken out of service?',
        c: ['Their control system and cabling are being replaced.', 'A new tenant requested the change.',
            'They are being removed permanently.', 'They failed a recent safety inspection.'],
        a: 0,
        e: '制御系統と配線を全面更新するためと明記されている。',
        w: ['正解。', 'テナントの要望ではない。', '恒久的な撤去ではない。', '検査不合格の話はない。'] },
  ...
  sp({
    ...
    q: [
      { tag: '詳細', s: 'What problem is described in the messages?',
        c: ['A caterer has been delayed by a traffic accident.', 'A venue has double-booked the event.',
            'A menu item is out of stock.', 'A payment has not been processed.'],
        a: 0,
        e: '「環状道路で事故があり、その渋滞に巻き込まれている」というケータリング業者の遅延について。',
        w: ['正解。', '会場の重複予約の話はない。', '在庫切れの話ではない。', '支払いの話は出ていない。'] },
      { tag: '詳細', s: 'What does Mr. Wysocki ask Ms. Achebe to do?',
        c: ['Ask the caterer to switch to the cold menu before loading the van.', 'Find a replacement caterer.',
            'Delay the start of seating.', 'Cancel the catering order entirely.'],
        a: 0,
        e: '「今すぐ電話して、積み込む前に冷製メニューに切り替えるよう伝えて」と依頼している。',
        w: ['正解。', '別業者の手配は依頼していない。', '開始時刻の変更は述べていない。', '注文の取り消しではない。'] },
    ],
  }),
  ...
  sp({
    ...
    q: [
      ...
      { tag: '詳細', s: 'Why did the company reject hiring further ahead of peak season?',
        c: ['Suitable candidates were not available that early in the market.', 'It would have exceeded the training budget.',
            'It had failed at a competitor before.', 'It was prohibited by employment regulations.'],
        a: 0,
        e: '「その時期にはまだ適した候補者が市場にいなかった」ため見送られた。',
        w: ['正解。', '予算超過の話も出ていない。', '競合の事例には触れていない。', '規制の話はない。'] },
