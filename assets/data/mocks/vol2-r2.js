  ...
  sp({
    ...
    q: [
      { tag: '詳細', s: 'What is stated about lane swimming during the closure?',
        c: ['It will be cancelled entirely.', 'It will have no time limit.',
            'It will be held at a nearby centre.', 'It will require a prior booking.'],
        a: 3,
        e: '「アプリで事前予約が必要で、当日の飛び込み利用はできない」と明記されている。',
        w: ['中止ではなく学習用プールに移す。', '第2段落は sessions will be limited to forty-five minutes と、1 回 45 分に制限されることを明記している。時間制限が無いとするこの記述は本文と正面から矛盾するので偽。', '別施設への移動ではない。', '正解。'] },
    ...
    q: [
      { tag: '詳細', s: 'What did Halloway discover about its returns?',
        c: ['They were higher than the industry average.', 'They came mainly from a small group of new customers.',
            'They differed sharply between two ordering behaviours.', 'They were not reduced after the change.'],
        a: 2,
        e: '同一商品を複数サイズ注文する客とそうでない客で、返品率が 11 パーセントと 58 パーセントに分かれた。',
        w: ['業界平均との比較はしていない。', '新規顧客の話ではない。', '正解。', '第5段落は Total returns fell by nine percentage points と、是正後に総返品が 9 ポイント下がったことを明記している。これと正面から矛盾するので偽。'] },
