    ...
    q: [
      ...
      { tag: '詳細', s: 'What is one option offered to the listener?',
        c: ['Having the check-up on Thursday and the clean later that day with another hygienist.', 'Having the entire appointment cancelled and a full refund paid back within ten working days.',
            'Moving the whole appointment to a day when Ms. Adeyemi is still away.', 'Switching permanently to Mr. Okafor for all future cleans and check-ups.'],
        a: 0,
        e: '検診は木曜のまま、清掃は別の衛生士が同日午後3時半に担当する案が示されている。',
        w: ['正解。', '「どちらの案でも追加の請求はない」と述べているだけで、支払い済みの費用も返金も話題になっていない。', '音声は "I can move the whole appointment to the following Tuesday, when she\'s back" と述べており、予約全体を移す案の移動先はアデイェミ氏が復帰している日である。彼女が不在のままの日へ移すとするこの記述は音声と正面から矛盾するので偽。', 'オカフォー氏が担当するのは今回の清掃だけで、以後ずっと担当を替えるという話ではない。'] },
    ...
    q: [
      ...
      { tag: '推測', s: 'What is implied about passengers who are reseated?',
        c: ['They must request a new seat at the service desk.', 'They will receive a new boarding pass automatically at the gate.',
            'They will have to take a later flight departing the next day.', 'They will not board from gate 14 with the others.'],
        a: 1,
        e: '「対象者には搭乗口で新しい搭乗券が自動的に発行され、サービスデスクに行く必要はない」とある。',
        w: ['サービスデスクへの訪問は不要と明記。', '正解。', '便の変更ではない。', '放送は "this flight will now depart from gate 14 instead of gate 22" と述べており、この便の乗客は全員 14 番搭乗口から搭乗する。座席が変わった乗客だけ別扱いになるとするこの記述は本文と正面から矛盾する。'] },
    ],
  }),
    ...
    q: [
      ...
      { tag: '詳細', s: 'What is said about reporting a new incident with no injuries?',
        c: ['It can only be reported by telephone during the company\'s normal business hours.', 'It cannot be started until a claims handler has returned the customer\'s call.',
            'It does not require a callback from a claims handler to finish the process.', 'It can be started online and avoids the queue for the first stage.'],
        a: 3,
        e: '「オンラインで開始でき、約 10 分で最初の段階の待ち行列を回避できる」と述べている。',
        w: ['オンラインでの受付に時間帯の制限は付いておらず、営業時間内に限るという条件は述べていない。', '担当者からの折り返しは手続きを「完了させる」ためのもので、報告そのものはその前にオンラインで開始できると明言している。', '音声は "A claims handler will still call you back within one working day to finish the process." と述べており、手続きを完了させるには担当者からの折り返しがある。折り返しが不要だとするこの記述は本文と正面から矛盾する。', '正解。'] },
    ...
    q: [
      ...
      { tag: '詳細', t: ['p4type'], s: 'Why is the swap necessary?',
        c: ['A delivery needs to be stored before it warms up.', 'A staff member called in sick this morning.',
            'The grill station is being repaired by an outside contractor today.', 'Tomorrow\'s rota cannot be staffed as written.'],
        a: 0,
        e: '「生樽の配達が遅れており、庭先で温まる前にすぐ貯蔵室に運び込む必要がある」が理由。',
        w: ['正解。', '欠勤者が出たという説明はない。交代は最初の 1 時間だけで、通常の持ち場はその後戻る。', 'グリル台の故障や修理には触れていない。交代の理由は樽の配達である。', '話し手は "tomorrow\'s rota is unaffected" と述べており、明日の勤務表はそのまま回る。明日の勤務表が掲示どおりには埋まらないとするこの記述は本文と正面から矛盾する。'] },
