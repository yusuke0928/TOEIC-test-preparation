  ...
  /* ── 4. 求人・案内：結束性と冠詞 ───────────────────── */
  {
    ...
    questions: [
      { id: 'p6c-04-1', topics: ['pron', 'cohesion'],
        ...
        answer: 2,
        exp: 'before 以下は reach を述語とする節なので、空所は主語の位置。主節の目的語 anomalies（複数）を、同じ文の従属節でそのまま受け直す純粋な照応なので、複数の主格人称代名詞 they が入る。',
        why: ['単数。受ける名詞 anomalies は複数で数が一致しない。',
              '指示代名詞 those は、those who / those that / those of ... のように後ろに修飾語を伴って「〜する人・もの」を表すか、他と対比して指し示すときに使う語。直前の名詞をそのまま引き継ぐこの位置には立たない。',
              '正解。複数名詞 anomalies を受ける主格の人称代名詞。',
              '指示代名詞 these は既出のものを受けられるが、対比したり改めて取り上げ直したりする働きを伴う語。同一文内で直前の目的語をそのまま引き継ぐ照応には人称代名詞を使う。'],
  ...
  /* ── 5. 社内メモ：時制と語彙 ───────────────────────── */
  {
    ...
    questions: [
      ...
      { id: 'p6c-05-2', topics: ['ctense'],
        ...
        answer: 2,
        exp: 'メモの日付は 11 月 8 日で、10 月の地域会議はすでに終わっている。決め手は次の文の対比で、「For everyone else（それ以外の全員には）20 分の録画解説を今インターネットに出した」とある。録画は会議に出られなかった人のための代替なので、出席者は会議の場ですでに実物を見ている。10 月という済んだ一時点の出来事なので過去形。',
        why: ['現在形。10 月の会議という一度きりの済んだ出来事であり、現在の習慣でも一般的事実でもない。',
              '未来形。会議が終わっていること自体は未来の受け取りを妨げないが、次文が「それ以外の全員には録画を用意した」と、出席者はすでに見たという前提で対比している。出席者だけ後日に見るという読みは成り立たない。',
              '正解。10 月の会議という過去の一時点で受け取った出来事。',
              '現在完了進行形。「今まで繰り返し受け取り続けている」という意味になり、10 月の会議1回に限られる出来事を表せない。'],
